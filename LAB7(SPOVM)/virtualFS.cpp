...
{
	if (findDirBlock(dirName))
		return;
	DirsBlock dirsBlock;
	readBlock(curDirDesc.dirsBlockAdr, (char*)&dirsBlock);
	while (dirsBlock.nextDirsBlock)
...
void VirtualFS::deleteDir(string dirName)
{
	...
	for (auto it : tempFileNames)
		deleteFile(it);
	for (unsigned long i = 1; i < tempDirNames.size(); i++)
		deleteDir(tempDirNames[i]);
	enterDir("..");
	...
	for (char i = 0; i < 31; i++)
		if (dirsBlock.dirs[i])
		{
			...
			if (string(dirDesc.name) == dirName)
				numberToDelete = i;
		}
...
{
	if (findDirBlock(newName) || !findDirBlock(dirName))
		return;
	...
	while (true)
	{
		for (char i = 0; i < 31; i++)
			if (filesBlock.files[i])
			{
				...
				if (string(fileDesc.name) == fileName)
					return result;
			}
			else
				return 0;
...
{
	if (findFileBlock(fileName))
		return;
	FilesBlock filesBlock;
	readBlock(curDirDesc.filesBlockAdr, (char*)&filesBlock);
	while (filesBlock.nextFilesBlock)
		readBlock(filesBlock.nextFilesBlock, (char*)&filesBlock);
	while (true)
	{
		for (char i = 0; i < 31; i++)
...
{
	if (findFileBlock(newName) || !findFileBlock(fileName))
		return;
...
void VirtualFS::editFile(string fileName, string newData)
{
	...
	{
		for (char j = 0; j < 124 && i + j < newData.size(); j++)
			dataBlock.data[j] = newData[j + i];
		if (i + 124 < newData.size())
		{
			if (!dataBlock.nextDataBlock)
				dataBlock.nextDataBlock = allocateBlock();
